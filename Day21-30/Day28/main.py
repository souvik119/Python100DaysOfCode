...
def reset_timer():
    ...
    #set text back to 00:00
    canvas.itemconfigure(timer_text, text="00:00")
    #reset checkmarks
    check_label.config(text="")
    #reset title text back to Timer
...
def start_timer():
    ...
    # * 60 converting to seconds

    # If its first/third/fifth rep then work sec if 8th rep then long break if 2nd 4th then short break
    if reps % 8 == 0:
...
def count_down(count):
    ...
    if count > 0:
        ...
        start_timer()
        #add a check mark for every 2 reps completed
        mark = ""
